...
def generate_data(data_size, mode='square'):
    ...
    return np.array(grid_list), np.array(robot_pos_list), np.array(time_list), np.array(action_list),  [np.array(feat_list), np.array(adj_list), np.array(label_list)]


if __name__ == '__main__':
    argparser = argparse.ArgumentParser(description="This module generates data for testing target coverage. The data is saved as pickle files and includes grid+target configuration, robot positions, action slected by global centralized greedy algorithm, time taken to choose those actions, and the data for GNN predictions (features, adjacency matrix, labels)")
    argparser.add_argument('--batch_size', type=int, default=1000, help="Number of test instances")
    argparser.add_argument('--save_path', type=str, default=None, help="Location whther data will be saved")
    argparser.add_argument('--mode', type=str, default='rect', help="Type of FoV, `rect`(default) or `sqaure`")
    ...
    pickle.dump(action_data, open(f'{args.save_path}/action_data.pkl', 'wb'))
    pickle.dump(model_data, open(f'{args.save_path}/model_data.pkl', 'wb'))
